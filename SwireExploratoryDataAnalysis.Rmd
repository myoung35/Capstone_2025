...
Table of contents
Headers and code block titles: 
Intro : State the project goal and summarize the business and analytic problems. State the purpose of the EDA notebook and the questions about the data that you will be exploring. 
Data description: describe the available data for the project 
Scope of missing data and how we will fix it
EDA visuals with titles and axis labels 
Results where we summarize the findings
Compilation to HTML with professional format
Group member contribution break down to the note book 
...
```
It does seem obvious that customers below the order threshold of 400 gallons would order less, we will continue to explore further what kinds of traits customers have that make them more or less likely to order above/below threshold.
...
**aggregated_cost_wide**: This dataset transforms AggregatedCost into a wide format, where each year’s data is placed into separate columns instead of separate rows. Each row represents a unique CUSTOMER_NUMBER. Instead of multiple rows for different years, the years are now column headers.
...
grid.arrange(plot2, plot5, ncol = 2, nrow = 2)
...
Analysis of Order Distribution: Regular vs. Log-Transformed Visualizations
The two histograms provide critical insights into customer ordering behavior, revenue concentration, and business risks by showing the raw order distribution and its log-transformed counterpart.

1️⃣ Identifying Customer Segments & Purchase Behavior
The regular histogram (left) is highly right-skewed, showing that most customers place small orders, while a few customers have extremely large orders.
The log-transformed histogram (right) presents a more balanced, bell-shaped distribution, revealing a clear mid-tier customer segment that was previously hidden.
The spike at 0 in both graphs suggests that many customers placed no orders in 2023.
🔹 Business Implications:

Customer Segmentation: The business should classify customers into small, mid-tier, and high-volume buyers to tailor marketing and sales strategies.
Revenue Concentration: If a few customers dominate sales, losing them could pose a significant business risk.
Engagement Opportunities: Customers with zero orders might be churned, inactive, or in need of targeted incentives to encourage reordering.
2️⃣ Outliers & Business Risk Detection
The regular histogram obscures smaller order values, making it difficult to see differences in customer behavior.
The log-transformed histogram compresses large values, making variability in mid-range customers more apparent.
If only a small number of customers contribute the majority of revenue, the company could face a high customer dependency risk.
🔹 Business Implications:

Customer Concentration Risk: If a handful of accounts are responsible for most sales, the business should diversify its customer base to reduce dependency.
Outlier Detection: Spotting extreme orders can help identify potential fraud, bulk-buying trends, or operational inefficiencies.
Zero Order Customers: A deeper dive is needed to determine if these are inactive customers, lost clients, or seasonal buyers.
3️⃣ Forecasting & Demand Planning
The regular histogram suggests extreme variance in orders, making it challenging to predict future demand.
The log-transformed histogram normalizes the data, making it more suitable for statistical modeling and forecasting.
The distribution suggests a log-normal order pattern, which can be used to optimize inventory and marketing strategies.
🔹 Business Implications:

Improved Demand Forecasting: Using log-transformed data helps predict order volume more accurately.
Better Inventory Management: A data-driven approach can help allocate stock more efficiently between high- and low-volume customers.
Targeted Growth Strategies: The company should analyze whether mid-tier customers can be moved up to higher spending levels.
...
Main_Customer_Data <- Main_Customer_Data |>
  mutate(ThreshBins = case_when(
    TwoYearTotal < 10 ~ "Below Threshold",
...
```{r Co2 5}
# Count CO₂ vs. Non-CO₂ customers among those who met the threshold
