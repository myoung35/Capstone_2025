...
```{r}
parent_cust_orders %>% 
  mutate(hasOutlet = case_when(Parent))
```


 FSR - Misc is full service restaurant. Both below and above thresh have the most customers in this sub group. Second largest customer in above thresh is Other Outdoor Activety, which the fifth largest group in the under thresh. Meaning this group is typically over thresh? 
 
 Other Licensesd Hospitatlity is third largest customer group that is below thresh, but above they are the 8th largest in above. Which means that this group is typically under thresh?
 
 might be better to look at proportions....
...
Things I want to do
- Add an has outlet column 
  - How many outlets perform well 
- Do proportions in the subgroups 
- Look at if a customer is above threshold compared to their onboarding date
- Look at local market partner (local market partner that buy only fontain )
